...
const UrlModal: React.FC<UrlModalProps> = ({ newUrl, setNewUrl }) => {
    ...
    const handleCopyToClipboard = async () => {
        try {
            await navigator.clipboard.writeText("https://paster-fqin.vercel.app/" + newUrl);
    ...
    return (
        <div className="bg-[#00000063] fixed top-0 left-0 w-[100vw] h-[100vh]">
            <div className="bg-[#ff3c87d0] rounded-lg px-3 fixed relative top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-[40%] h-[15%] flex justify-center items-center">
                <input type="text" value={"https://paster-fqin.vercel.app/" + newUrl} className="outline-none rounded-s-md text-black w-[80%] p-2 text-center" /><button onClick={handleCopyToClipboard} className="rounded-e-md  w-[10%] p-2 text-center bg-gray-800">copy</button>
